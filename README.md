...
- `GEMINI_MODEL`: optional, defaults to `gemini-3.1-flash-lite` (15 RPM free tier). Set to `gemini-3.5-flash` for smarter analysis at 5 RPM (~13s between holdings).
- `GEMINI_REQUEST_GAP_MS`: optional override for milliseconds between analyze calls (auto-set from model if omitted).
